...
const PitchTuningNeedle = () => {
  console.log('Rendering PitchTuningNeedle component');
  ...
  const rotation = ((pitch - minPitch) / (maxPitch - minPitch)) * 180 - 90;
  ...
  return (
    <div>
      <svg width="300" height="200" viewBox="-150 -100 300 200">
        ...
        {/* Labels */}
        <text x="-110" y="30" textAnchor="middle">{minPitch}</text>
        <text x="0" y="30" textAnchor="middle">{(minPitch + maxPitch) / 2}</text>
        <text x="110" y="30" textAnchor="middle">{maxPitch}</text>
      </svg>
      <div>
        <p>Current Pitch: {pitch.toFixed(2)} Hz</p>
